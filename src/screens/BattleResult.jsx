...
export default function BattleResult() {
  ...
  useEffect(() => {
    ...
    const t2 = setTimeout(() => {
      ...
      if (!savedResult.local) g.refreshStats().catch(e => g.notice(e.message))
    }, 1500)
    ...
    <Page>
      ...
      <Panel className="absolute left-[25px] top-[105px] w-[415px] h-[675px] p-5" initial="hidden" animate="show">
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          <Card className="flex-1 h-[110px] p-2 flex items-center gap-2" style={{ borderColor: '#a855f7' }}><span className="flex-1 text-center leading-none"><span className="block text-[13px] font-extrabold text-ink-3 uppercase">Missed</span><span className="block font-display font-extrabold text-[50px] text-purple-500">{bs}</span></span><img src={bot.img} alt="" className="w-[64px] h-[64px] rounded-2xl object-cover object-top" /></Card>
        </div>
        <div className="mt-3 pill h-[44px] justify-center gap-4 text-[15px] font-bold text-ink-2"><span className="flex items-center gap-1"><Timer size={16} /> {rounds} rounds</span><span className="w-px h-5 bg-[var(--line)]" /><span className="flex items-center gap-1"><Clock size={16} /> {clock}</span></div>
      ...
      <Stack className="absolute left-[545px] top-[115px] w-[600px] text-center" start={0.3}>
        <Item v="pop"><h1 className="font-display font-extrabold text-[76px] leading-none uppercase" style={{ background: 'linear-gradient(90deg,#a855f7,#ec4899)', WebkitBackgroundClip: 'text', color: 'transparent' }}>Practice complete</h1></Item>
        <Item className="mt-2 font-display font-extrabold text-[34px] text-ink">{name}: <span className="text-sky-500">{me} correct</span> <span className="text-[26px] text-ink-3">·</span> <span className="text-purple-500">{bs} missed</span></Item>
      </Stack>
      ...
      <Panel className="absolute left-[1245px] top-[105px] w-[400px] h-[655px] p-5" initial="hidden" animate="show">
        <div className="flex items-center gap-3"><img src="/art/hd/nova-v2.webp" alt="" className="w-[80px] floaty" /><div className="card px-4 py-3 text-[15px] font-bold text-ink-2 leading-snug">You answered {me} of {rounds} {pkg.mission.title} questions correctly. {bot.name} is a game character, not another learner.</div></div>
        <div className="mt-4 font-display font-extrabold text-[15px] text-ink uppercase tracking-wide">What you did well</div>
        <div className="mt-2 grid grid-cols-2 gap-3">{(me > 0 ? [[Target, '#ef4444', `${me} of ${rounds} rounds won`], [PieChart, '#3b82f6', `${pkg.mission.title} practised`]]
          : [[Target, '#ef4444', 'You played every round'], [PieChart, '#3b82f6', `${pkg.mission.title} practised`]]).map(([I, c, t]) => <Card key={t} className="h-[90px] p-3 flex items-center gap-3"><span className="icon-orb w-[46px] h-[46px]" style={{ color: c, background: `${c}1f` }}><I size={24} /></span><span className="text-[15px] font-bold text-ink leading-snug">{t}</span></Card>)}</div>
        <div className="mt-4 font-display font-extrabold text-[15px] text-ink uppercase tracking-wide">Optional next step</div>
