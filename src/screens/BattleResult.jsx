  ...
  return (
    <Page>
      ...
      <Stack className="absolute left-[545px] top-[115px] w-[600px] text-center" start={0.3}>
        <Item v="pop"><h1 className="font-display font-extrabold text-[96px] leading-none uppercase" style={{ background: 'linear-gradient(90deg,#a855f7,#ec4899)', WebkitBackgroundClip: 'text', color: 'transparent' }}>{won ? 'You won!' : drew ? 'A draw!' : 'So close!'}</h1></Item>
      ...
      <Panel className="absolute left-[1245px] top-[105px] w-[400px] h-[655px] p-5" initial="hidden" animate="show">
        <div className="flex items-center gap-3"><img src="/art/hd/nova-v2.webp" alt="" className="w-[80px] floaty" /><div className="card px-4 py-3 text-[15px] font-bold text-ink-2 leading-snug">{won ? `You had ${bot.name} on the fractions today \u2014 ${me} rounds to ${bs}.` : drew ? `Dead level, ${me} each. One more round would have settled it.` : `${bot.name} edged the fractions ${bs} to ${me}. We will take the rematch.`}</div></div>
        <div className="mt-4 font-display font-extrabold text-[15px] text-ink uppercase tracking-wide">What you did well</div>
        <div className="mt-2 grid grid-cols-2 gap-3">{(me > 0 ? [[Target, '#ef4444', `${me} of ${rounds} rounds won`], [PieChart, '#3b82f6', 'Fractions practised']]
          : [[Target, '#ef4444', 'You played every round'], [PieChart, '#3b82f6', 'Fractions practised']]).map(([I, c, t]) => <Card key={t} className="h-[90px] p-3 flex items-center gap-3"><span className="icon-orb w-[46px] h-[46px]" style={{ color: c, background: `${c}1f` }}><I size={24} /></span><span className="text-[15px] font-bold text-ink leading-snug">{t}</span></Card>)}</div>
        <div className="mt-4 font-display font-extrabold text-[15px] text-ink uppercase tracking-wide">Next recommendation</div>
        <Card hover className="mt-2 h-[80px] px-4 flex items-center gap-3" onClick={() => nav('/missions/fractions')}><span className="icon-orb w-[46px] h-[46px] text-orange-500" style={{ background: '#ffedd5' }}><Timer size={24} /></span><span className="flex-1 leading-tight"><span className="block font-extrabold text-[17px] text-primary-ink">Fraction Mission</span><span className="block text-[13px] font-semibold text-ink-3">Sharpen your fraction skills.</span></span><Chev size={22} className="text-ink-3" /></Card>
