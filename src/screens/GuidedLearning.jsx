...
function lessonSteps(pkg) {
  ...
  return fallbacks.map((fallback, index) => {
    ...
    return {
      ...
      alternate: step.alternate_teaching_text || '',
...
export default function GuidedLearning() {
  ...
  const pages = splitPages(alternate && step.alternate ? step.alternate : step.body)
  const page = Math.min(pageIndex, pages.length - 1)
  const mini = step.mini
  ...
  const previous = () => {
    ...
    else if (stepIndex > 0) { setStepIndex(stepIndex - 1); setPageIndex(0); setAlternate(false) }
  ...
  const next = () => {
    ...
    else { setStepIndex(stepIndex + 1); setPageIndex(0); setAlternate(false) }
  }
  ...
  return <Page>
    ...
    <main className="kv-learning">
      ...
      <section className={`kv-learning__canvas kv-learning__canvas--${stepIndex}`} aria-label={`${STEP_NAMES[stepIndex]} lesson`}>
        ...
        <div className="kv-learning__title-row"><h1>{step.title}</h1><button type="button" onClick={() => speak(`${step.title}. ${step.body}`)} aria-label="Hear lesson"><Volume2 size={24} /></button></div>
        <div className={`kv-learning__media-row ${step.image ? '' : 'kv-learning__media-row--text-only'}`}>
          <div className="kv-learning__written">
            <div className="kv-learning__teaching"><span>{stepIndex === 0 ? 'Understand the idea' : stepIndex === 1 ? 'Follow the example' : 'Remember the rule'}</span><p className="kv-learning__body">{pages[page]}</p></div>
            <div className="kv-learning__idea"><Lightbulb size={28} /><div><small>{stepIndex === 2 ? 'Take this into the quiz' : 'The big idea'}</small><strong>{step.key}</strong></div></div>
          </div>
          <StepMedia key={`${stepIndex}:${step.image}`} step={step} index={stepIndex} />
        </div>
        {stepIndex === 1 && <button type="button" className="kv-learning__alternate" onClick={() => { setAlternate(value => !value); setPageIndex(0); sfx.tap() }}><RefreshCw size={18} />{alternate ? 'Back to the example' : step.alternate ? 'Try another way' : 'See the idea again'}</button>}
