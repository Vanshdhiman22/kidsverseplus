...
import { Flame, Bell, Award , Lock} from 'lucide-react'
  ...
  return (
    <Page>
      ...
      <Panel className="absolute left-[640px] top-[420px] w-[1000px] h-[365px] p-6" initial="hidden" animate="show">
        ...
        <Stack className="mt-3 grid grid-cols-4 gap-8" start={0.8} delay={0.12}>{miles.map(([I, c, t, s, got], i) => <Item key={t} v="pop"><Card className={cn('relative h-[240px] p-4 pt-8 flex flex-col items-center text-center', !got && 'opacity-55')}><span className="absolute -top-4 -left-3 w-[42px] h-[42px] rounded-full grid place-items-center text-white font-display font-extrabold text-[20px]" style={{ background: 'var(--grad-primary)' }}>{i + 1}</span><span className="icon-orb w-[110px] h-[110px]" style={{ color: got ? c : 'var(--ink-3)', background: got ? `${c}1f` : 'var(--lavender)' }}>{got ? <I size={56} /> : <Lock size={44} />}</span><span className="mt-3 text-[17px] font-extrabold text-ink leading-tight">{t}</span><span className="mt-1 text-[13px] font-semibold text-ink-3 leading-tight">{s}</span></Card></Item>)}</Stack>
        <div className="relative mt-5 mx-[100px] h-[4px] rounded-full bg-[var(--lavender-2)]"><motion.div className="absolute inset-y-0 left-0 rounded-full" style={{ background: 'var(--grad-primary)' }} initial={{ width: 0 }} animate={{ width: `${(earned / miles.length) * 100}%` }} transition={{ duration: 1.4, delay: 0.3 }} />{[0, 1, 2, 3].map(i => <motion.span key={i} className="absolute -top-[8px] w-[20px] h-[20px] rounded-full border-4 border-[var(--primary)] bg-white" style={{ left: `calc(${i * 33.3}% - 10px)` }} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.1 + i * 0.3 }} />)}</div>
      </Panel>
