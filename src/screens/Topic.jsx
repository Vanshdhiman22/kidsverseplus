...
import { TOPIC_STEPS, gradeLabel } from '../data/catalog.js'
  ...
  return (
    <Page>
      ...
      <Stack className="absolute left-[50px] top-[115px] w-[500px]" start={0.2}>
        ...
        <Item className="flex items-center gap-4"><h1 className="font-display font-extrabold text-[86px] leading-none text-ink">Fractions</h1><img src="/art/planet-sm.webp" alt="" className="w-[70px] floaty" /></Item>
        <Item className="text-[26px] font-extrabold text-primary-ink">{gradeLabel(grade)} <span className="text-ink-3">•</span> Maths</Item>
        <Item className="mt-2 text-[18px] font-semibold text-ink-2 leading-snug">Understand parts of a whole, equivalent fractions, compare and order, and solve real-world problems.</Item>
      </Stack>
      <Stack className="absolute left-[45px] top-[345px] flex flex-col gap-3" start={0.6} delay={0.1}>
        {[[BookOpen, 'School Syllabus', ['Equivalent fractions', 'Compare and order'], 'Completed', 'done', '#3b82f6'], [Box, 'Kidsverse Plus', ['Visual models', 'Applications', 'Word problems'], 'In progress', 'progress', '#8b5cf6'], [Trophy, 'Competition Edge', ['Higher-order reasoning'], 'Locked', 'locked', '#f59e0b']].map(([I, t, items, st, k, c]) => (
      ...
      <div className="absolute left-[1310px] top-[180px]"><SpeechBubble tail="left" text="Equivalent fractions show the same value even though the numerator and denominator change! ✨" delay={0.3} className="w-[240px] text-[17px]" /></div>
