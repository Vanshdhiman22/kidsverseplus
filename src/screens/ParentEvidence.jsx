...
export default function ParentEvidence() {
  ...
  /* The button beside the title played a tap sound and did nothing else, so a parent asking
  ...
  const evidence = [
    lastTest
      ? ['Latest test', lastTest.correct / lastTest.total, `${lastTest.correct}/${lastTest.total}`,
         lastTest.correct / lastTest.total >= 0.6 ? '#22c55e' : '#f59e0b',
         lastTest.correct / lastTest.total >= 0.6 ? 'Great job!' : 'Worth revisiting',
          ...
          <div className="flex flex-col gap-5">
            <Card className="p-6 flex gap-6">
              <div className="flex-1"><div className="font-display font-extrabold text-[22px] text-ink">{name}'s recent performance</div>
                <div className="mt-4 grid grid-cols-2 gap-4">{/* Was a fixed 8/10 and 2/5. The first tile is the child's most recent test, banked when
